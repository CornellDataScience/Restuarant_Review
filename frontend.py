...
from dbms_visualization import get_all_res_review_trends, get_one_res_review_trend, choose_res_ids, visualize_review_score
from dbms_visualization import visualize_res_review_trends_graph, visualize_yelp_competitor_score, visualize_NLP_review_trends
from dbms import yelp_rest_name_to_id_dict, yelp_id_restaurant_dict, get_res_avg_rating
import io
import collections
import base64
...
def img_data(rest_id):
	...
	# plt.close()
	img = io.BytesIO()
	visualize_review_score(rest_id)
	plt.savefig(img,format='png')
	img.seek(0)
	plot_url = base64.b64encode(img.getvalue()).decode()
	plt.close()
